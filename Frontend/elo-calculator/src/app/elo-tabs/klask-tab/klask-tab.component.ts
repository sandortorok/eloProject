...
@Component({
  ...
  templateUrl: './klask-tab.component.html',
  styleUrls: ['./klask-tab.component.scss']
})
export class KlaskTabComponent implements OnInit {
  players:Player[] = [];
  playing:string[] = [];
  ...
  constructor(private dataservice: DataService, private httpservice: HttpService, private modalService: NgbModal) {
    this.players = dataservice.klaskPlayers;
  ...
  playMatch(){
    ...
    this.httpservice.updateKlaskPlayer('rating', {name: this.players[idx1].name, rating: this.players[idx1].rating})
      .subscribe(res=>{})

    this.httpservice.updateKlaskPlayer('rating', {name: this.players[idx2].name, rating: this.players[idx2].rating})
      .subscribe(res=>{})

    this.httpservice.updateKlaskPlayer('games', {name: this.players[idx1].name, games: this.players[idx1].games+=1})
      .subscribe(res=>{})

    this.httpservice.updateKlaskPlayer('games', {name: this.players[idx2].name, games: this.players[idx2].games+=1})
      .subscribe(res=>{})
    ...
    this.httpservice.addKlaskGame(gameobj)
      .subscribe(res=>{})
  ...
  addPlayer(){
    ...
    this.httpservice.createKlaskPlayer(this.addedPlayer)
      .subscribe(data=>{})
    this.addedPlayer=""
  }
  onDelSelect(name){
  ...
  deletePlayer(){
    ...
    this.httpservice.deleteKlaskPlayer(this.deletedPlayer)
      .subscribe(res=>{
        console.log(res);
      })
